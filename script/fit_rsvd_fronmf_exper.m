...
outdir = '../bigdata';
factorsoutfile  = join([dataname, '_factors_rsvdfronmf_d',num2str(d),'.csv']);
loadingsoutfile = join([dataname, '_loadings_rsvdfronmf_d',num2str(d),'.csv']);
erroutfile = join([dataname, '_error_rsvdfronmf_d',num2str(d),'.csv']);
% SET UP ENVIRONMENT
% ------------------
addpath ../code
...
tic;
[A B f1 f2] = fronmf_exper(counts,countshat,L0,F0',1e-06,1000);
timing = toc;
